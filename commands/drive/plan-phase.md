...
**Orchestrator role:** Parse arguments, validate phase, research domain (unless skipped or exists), spawn drive-planner agent, verify plans with drive-plan-checker, iterate until plans pass or max iterations reached, present results.
...
**If not found:** Error - user should run `/drive:new-project` first.
...
```sh
FLOW_RESEARCH=$(cat .planning/config.json 2>/dev/null | grep -o '"research"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false' || echo "true")
```

**If `flow.research` is `false` AND `--research` flag NOT set:** Skip to step 6.
...
```sh
---
Aria Drive > RESEARCHING PHASE {X}
---
...
```
Task(
  prompt="First, read ~/.claude/agents/drive-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
## 8. Spawn drive-planner Agent

Display stage banner:

```sh
---
Aria Drive > PLANNING PHASE {X}
---
...
```
Task(
  prompt="First, read ~/.claude/agents/drive-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
...
- Check config: `FLOW_PLAN_CHECK=$(cat .planning/config.json 2>/dev/null | grep -o '"plan_check"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false' || echo "true")`
- If `flow.plan_check` is `false`: Skip to step 13
...
## 10. Spawn drive-plan-checker Agent

Display:

```sh
---
Aria Drive > VERIFYING PLANS
---
...
Task(
  ...
  subagent_type="drive-plan-checker",
  model="{checker_model}",
...
Spawn drive-planner with revision prompt:
...
- /drive:plan-phase {X} --research -- re-research first
...
- [ ] drive-phase-researcher spawned if research needed
- [ ] Existing plans checked
- [ ] drive-planner spawned with context (including research.md if available)
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] drive-plan-checker spawned (unless --skip-verify)
