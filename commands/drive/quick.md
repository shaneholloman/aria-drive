---
name: drive:quick
description: Execute a quick task with Aria Drive guarantees (atomic commits, state tracking) but skip optional agents
argument-hint: ""
allowed-tools:
  - Read
  - Write
...
<objective>
Execute small, ad-hoc tasks with Aria Drive guarantees (atomic commits, state.md tracking) while skipping optional agents (research, plan-checker, verifier).

Quick mode is the same system with a shorter path:

- Spawns drive-planner (quick mode) + drive-executor(s)
- Skips drive-phase-researcher, drive-plan-checker, drive-verifier
...
<execution_context>
Orchestration is inline - no separate flow file. Quick mode is deliberately simpler than full Aria Drive.
</execution_context>
...
Check that an active Aria Drive project exists:

```sh
if [ ! -f .planning/roadmap.md ]; then
  echo "Quick mode requires an active project with roadmap.md."
  echo "Run /drive:new-project first."
  exit 1
fi
```
...
Spawn drive-planner with quick mode context:
...
Spawn drive-executor with plan reference:
...
Aria Drive > QUICK TASK COMPLETE
...
Ready for next task: /drive:quick
```
