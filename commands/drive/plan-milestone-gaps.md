---
name: drive:plan-milestone-gaps
...
<objective>
Create all phases necessary to close gaps identified by `/drive:audit-milestone`.

Reads MILESTONE-audit.md, groups gaps into logical phases, creates phase entries in roadmap.md, and offers to plan each phase.

One command creates all fix phases -- no manual `/drive:add-phase` per gap.
</objective>

<execution_context>
<!-- Spawns drive-planner agent which has all planning expertise baked in -->
</execution_context>
...
```
No audit gaps found. Run `/drive:audit-milestone` first.
```
...
**Also available:**
- `/drive:execute-phase {N}` -- if plans already exist
...
`/drive:audit-milestone` -- re-audit to verify gaps closed
`/drive:complete-milestone {version}` -- archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/drive:plan-phase` next
</success_criteria>
