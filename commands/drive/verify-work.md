...
Output: {phase}-uat.md tracking all test results. If issues found: diagnosed gaps, verified fix plans ready for /drive:execute-phase
</objective>

<execution_context>
@~/.claude/drive/flows/verify-work.md
@~/.claude/drive/templates/uat.md
</execution_context>
...
8. If issues found:
   ...
   - Spawn drive-planner in --gaps mode to create fix plans
   - Spawn drive-plan-checker to verify fix plans
   - Iterate planner <-> checker until plans pass (max 3)
   - Present ready status with `/clear` then `/drive:execute-phase`
</process>
...
- /drive:execute-phase {Z+1} -- skip to execution (if already planned)
...
---
 Aria Drive > PHASE {Z} ISSUES FOUND [!]
---
...
/drive:execute-phase {Z} --gaps-only
...
- /drive:plan-phase {Z} --gaps -- regenerate fix plans
...
- /drive:plan-phase {Z} --gaps -- retry fix planning with guidance
- /drive:discuss-phase {Z} -- gather more context before replanning
...
- [ ] If issues: drive-planner creates fix plans from diagnosed gaps
- [ ] If issues: drive-plan-checker verifies fix plans (max 3 iterations)
- [ ] Ready for `/drive:execute-phase` when complete
</success_criteria>
