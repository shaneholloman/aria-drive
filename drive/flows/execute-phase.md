...
**No dependency analysis needed.** Wave numbers are pre-computed during `/drive:plan-phase`.
...
2. **Read files and spawn all autonomous agents in wave simultaneously:**
   ...
   @~/.claude/drive/references/checkpoints.md
   @~/.claude/drive/references/tdd.md
   </execution_context>
...
1. **Spawn agent for checkpoint plan:**
   ...
   ```
   Task(prompt="{subagent-task-prompt}", subagent_type="drive-executor", model="{executor_model}")
   ```
...
| `gaps_found`   | Present gap summary, offer `/drive:plan-phase {phase} --gaps` |
...
- `/drive:verify-work {X}` -- manual testing before planning
```

User runs `/drive:plan-phase {X} --gaps` which:

1. Reads verification.md gaps
2. Creates additional plans (04, 05, etc.) with `gap_closure: true` to close gaps
3. User then runs `/drive:execute-phase {X} --gaps-only`
...
Subagents: Fresh 200k each (full flow + execution).
...
1. Run `/drive:execute-phase {phase}` again
