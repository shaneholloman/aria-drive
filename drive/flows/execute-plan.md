...
@~/.claude/drive/references/git-integration.md
</required_reading>
...
<if mode="yolo">
```
AUTO: Auto-approved: Execute {phase}-{plan}-plan.md
...
2. Use Task tool with subagent_type="drive-executor" and model="{executor_model}":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="drive-executor" and model="{executor_model}":
...
```
WARNING Architectural Decision Needed
...
See `~/.claude/drive/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See ~/.claude/drive/references/checkpoints.md for complete checkpoint guidance.
</step>
...
If you're reading this flow because an orchestrator spawned you (vs running directly), the orchestrator's prompt will include checkpoint return instructions. Follow those instructions when you hit a checkpoint.
...
Create `.planning/phases/XX-name/{phase}-USER-setup.md` using template from `~/.claude/drive/templates/user-setup.md`.
...
Use ~/.claude/drive/templates/summary.md for structure.
...
WARNING Note: Issues were encountered during execution:
- [Issue 1]
- [Issue 2]
...
```
WARNING USER SETUP REQUIRED

This phase introduced external services requiring manual configuration:

PLANNED .planning/phases/{phase-dir}/{phase}-USER-setup.md
...
AUTO: Auto-continuing: Execute next plan ({phase}-{next-plan})
...
- `/drive:verify-work {phase}-{plan}` -- manual acceptance testing before continuing
...
- `/drive:verify-work {Z}` -- manual acceptance testing before continuing
- `/drive:discuss-phase {Z+1}` -- gather context first
...
- `/drive:verify-work` -- manual acceptance testing before completing milestone
- `/drive:add-phase <description>` -- add another phase before completing
