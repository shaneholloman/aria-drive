...
> **Note:** Planning methodology is in `agents/drive-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/drive/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/drive:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/drive/templates/user-setup.md` for full schema and examples
...
See `~/.claude/drive/flows/verify-phase.md` for verification logic.
